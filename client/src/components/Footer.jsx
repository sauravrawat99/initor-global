    ...
    <footer className="bg-navy-900 border-t border-white/10">
      ...
      <div className="container-max px-6 md:px-12 lg:px-20 py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        <div className="lg:col-span-1">
          ...
          <p className="text-slate-400 text-sm leading-relaxed mb-4">
            Empowering UK accountants with expert outsourced accounting services since 2006. GDPR compliant & ISO 27001 accredited.
          </p>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {services.map((s) => (
              <li key={s}>
                <Link to="/services" className="text-slate-400 text-sm hover:text-gold-400 transition-colors duration-200 flex items-center gap-2">
        ...
        <div>
          ...
          <ul className="space-y-2">
            {quickLinks.map((l) => (
              <li key={l.label}>
                <Link to={l.to} className="text-slate-400 text-sm hover:text-gold-400 transition-colors duration-200 flex items-center gap-2">
                  <span className="w-1 h-1 bg-gold-500 rounded-full" />
                  {l.label}
                </Link>
              </li>
        ...
        <div>
          ...
          <ul className="space-y-3 text-slate-400 text-sm">
            <li className="flex items-start gap-2">
              <svg className="w-4 h-4 text-gold-500 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          ...
          <div className="mt-5 flex gap-3">
            <span className="px-2 py-1 rounded border border-gold-500/40 text-gold-400 text-xs font-medium">GDPR</span>
            <span className="px-2 py-1 rounded border border-gold-500/40 text-gold-400 text-xs font-medium">ISO 27001</span>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-white/10 px-6 md:px-12 lg:px-20 py-5">
        <div className="container-max flex flex-col md:flex-row items-center justify-between gap-2 text-slate-500 text-xs">
          <p>© {new Date().getFullYear()} Initor Global. All rights reserved.</p>
